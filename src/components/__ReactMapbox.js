...
const ReactMapbox = () => {
   ...
   */
  const _getRouteCoords = async (busNumber: string) => {
    try {
      let buses: any = await firebase
        .firestore()
        .collection("buses")
        .doc(busNumber)
        .get();
      ...
      let routes: any = await firebase
        .firestore()
        .collection("routes")
        .doc(buses.data().routes[0])
        .get();
      if (!routes.data().exists) {
        console.log("No routes Found !");
        return;
      }
      let start: any = await firebase
        .firestore()
        .collection("stops")
        .doc(routes.data().source)
        .get();
      if (!start.data().exists) {
        console.log("No source stops Found !");
        return;
      }
      let end: any = await firebase
        .firestore()
        .collection("stops")
        .doc(routes.data().destination)
        .get();
      if (!end.data().exists) {
  ...
  const getRouteCoords = async () => {
    try {
        ...
        .then((bus: any) => {
          ...
          firebase
            ...
            .then(async (route: any) => {
              ...
              let stopn: [string] | [] = [];
              for (let i = 0; i < n; i++) {
                let response: any = await firebase
                  .firestore()
                  .collection("stops")
                  .doc(stops[i])
                  .get();
              ...
              let stop1: any = await firebase
                .firestore()
                .collection("stops")
                .doc(stops[6])
                .get();

              let stop2: any = await firebase
                .firestore()
                .collection("stops")
                .doc(routesArr[1])
                .get();
              ...
              finalData = [
                stop.data().coordinates,
                stop2.data().coordinates,
                stop1.data().coordinates,
              ];
      ...
      console.log(
        "Error Occured while getting the coords of route getRouteCoords fn : ",
        e
      );
    }
  };
  ...
  async function getRoute(start, end, mid, finalString) {
    ...
    const query = await fetch(
      `https://api.mapbox.com/directions/v5/mapbox/driving/${finalString.substring(
        0,
  ...
  * - destructure the longitude, latitude, uid and busNo from the
  * marker currentLocation
  * - adding the markers to the map
  * - set the coordinates of the marker
  * - and add the cuurent marker to the map
  */
  const _addMarkerToMap = (tempMarker) => {
    tempMarker.forEach((marker) => {
      const { longitude, latitude, uid, busNo } = marker.currentLocation;

      // adding the maker icon to the map
      var el = document.createElement("div");
      el.className = "custom-marker";
      el.key = uid;

      // redirect to bus info page when clicking on the marker
      el.onclick = function () {
        navigate(`/home/${busNo}`, { replace: true });
      };

      // set the coordinates of the maker 
      ...
      console.log("Marker Id : ", store);
    });
  };

  /*
   * Fetching the marker data from the firebase
   * - 
  */
