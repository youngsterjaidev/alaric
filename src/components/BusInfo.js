...
const Checkout = ({
  ...
  useEffect(() => {
    ...
    window
      .fetch("https://alaric-server.herokuapp.com/create-payment-intent", {
        method: "POST",
        headers: {
...
const Payment = ({
  ...
  const PayNow = async (e) => {
    console.log("Button is click", userInfo);
    try {
      const res = await axios.post("http://localhost:5050/decreaseBalance", {
        uid: user.uid,
        balance: userInfo.balance,
        amount: fare.distance * 2,
      });
              ...
              <ModalContent>
                ...
                <div style={{ padding: "1em" }}>
                  ...
                  >
                    ...
                    <div>{fare.origin || "Not selected yet"}</div>
                  </Form>
                  <Form
                    style={{
                      justifyContent: "space-between",
                  ...
                  >
                    ...
                    <div>{fare.destination || "Not selected yet"}</div>
                  </Form>
                  <Form
                    style={{
                      justifyContent: "space-between",
                  ...
                  >
                    ...
                    <div>{fare.distance || "Not selected yet"} KM</div>
                  </Form>
                  <Form
                    style={{
                      justifyContent: "space-between",
    ...
    <>
      {Object.keys(bus).length === 0 ? (
        ...
        <Container style={{ height: props.sidebarHeight }}>
          <HeaderThree>
            {bus.busNo} <Button onClick={() => setShowModal(true)}>Pay</Button>
          </HeaderThree>
          <div style={{ padding: "1em" }}>
            <div>{bus.busName}</div>
          </div>
